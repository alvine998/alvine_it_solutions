...
- Prefers a dark theme with `#0a0a14` base background, rgba-white text opacity hierarchy, purple/indigo gradient accents (#6366f1, #8b5cf6), large border-radius (12–16px), and subtle rgba-white borders for card separation. Expects all pages (including admin) to consistently use this palette — will flag cream/vanilla/light backgrounds as bugs. Confidence: 0.94
- Prefers the font pairing Space Grotesk for headings/display text and Inter for body/UI text. Confidence: 0.88
/dollar renderings anywhere. Explicitly flagged "Amount is in Rp" when dollar displays surfaced. Confidence: 0.95
...
- Prefers customer-facing integration examples (cURL / SDK snippets, integration docs) to work WITHOUT requiring login — use the customer's own API key (`sk-...`, from the API Keys page) directly rather than a session JWT / dashboard `$TOKEN`, so a customer with a valid key and credit balance can copy-paste and test immediately. Explicitly requested "use no login needed to improve at cURL integration." Confidence: 0.9
