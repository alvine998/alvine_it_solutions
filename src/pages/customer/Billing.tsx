...
const money = (n: number) => (n % 1 === 0 ? `$${n.toLocaleString()}` : `$${n}`);
...
export default function Billing() {
  ...
  const placeOrder = async (planId: string) => {
    ...
    try {
      ...
      setOk(`Order created for ${(plans.find(p => p._id === planId)?.name ?? "plan")}. Go to Orders to pay.`);
      await fetchAll();
    } catch (e: any) { setErr(e.message || "Failed to place order"); }
    finally { setPlacing(null); }
  };

  if (loading) return <div style={{ color: muted, fontFamily: "DM Mono, monospace", fontSize: 12 }}>Loading billing…</div>;

  return (
    <div style={{ display: "grid", gap: 16, color: fg }}>
      <div style={{ display: "flex", alignItems: "baseline", gap: 12, flexWrap: "wrap" }}>
        <h2 style={{ margin: 0, fontFamily: "Space Grotesk, sans-serif", fontSize: 18, fontWeight: 800, color: fg }}>Billing</h2>
        {activePlanId && <span style={{ fontFamily: "DM Mono, monospace", fontSize: 11, background: "rgba(99,102,241,0.12)", border: "1px solid rgba(99,102,241,0.25)", padding: "4px 9px", borderRadius: 20, color: "#6366f1" }}>active: {(plans.find(p => p._id === activePlanId)?.name ?? activePlanId).toUpperCase()}</span>}
        <span style={{ marginLeft: "auto", fontFamily: "DM Mono, monospace", fontSize: 11, color: subMuted, border: `1px solid ${border}`, padding: "5px 10px", borderRadius: 20, background: isLight ? "rgba(0,0,0,0.03)" : "rgba(255,255,255,0.03)" }}>Credits billed per 1k tokens · model: auto</span>
      </div>

      <p style={{ margin: 0, color: muted, fontSize: 13.5, lineHeight: 1.6 }}>
        Plans are live from <code style={{ fontFamily: "DM Mono, monospace", color: fg, background: isLight ? "rgba(0,0,0,0.06)" : "rgba(255,255,255,0.06)", padding: "2px 6px", borderRadius: 6 }}>GET /api/plans</code> — admin edits appear here. Pick a plan to create an order, then pay in <Link to="/dashboard/orders" style={{ color: "#6366f1" }}>Orders</Link>.
      </p>
      ...
      {plans.length === 0 ? (
        <div style={{ borderRadius: 14, border: `1px solid ${border}`, background: cardBg, padding: 18, fontFamily: "DM Mono, monospace", fontSize: 12, color: muted, lineHeight: 1.7 }}>
          No active plans yet. Create one in Admin → Plans. Hardcoded fallback prices: Starter $2/500, Pro $10/3.5k, Platinum $29/12k.
        </div>
      ) : (
            ...
            return (
              <div key={p._id} style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  {isActive && <span style={{ marginLeft: "auto", fontFamily: "DM Mono, monospace", fontSize: 10, letterSpacing: 0.6, padding: "4px 8px", borderRadius: 20, background: "rgba(16,185,129,0.14)", border: "1px solid rgba(16,185,129,0.25)", color: "#10b981" }}>CURRENT</span>}
                </div>
                ...
                ) : <div style={{ fontFamily: "DM Mono, monospace", fontSize: 11, color: subMuted }}>model: auto · billed per 1k tokens</div>}
                ...
                >
                  {isPlacing ? "Placing…" : isActive ? "Renew / extend →" : "Choose this plan →"}
                </button>
                <div style={{ fontFamily: "DM Mono, monospace", fontSize: 11, color: subMuted, textAlign: "center" }}>Creates pending order → pay in Orders</div>
              </div>
            );
          })}
        </div>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        <Link to="/dashboard/orders" style={{ fontFamily: "DM Mono, monospace", fontSize: 12, padding: "9px 14px", borderRadius: 10, background: isLight ? "#1c1917" : "rgba(255,255,255,0.08)", color: isLight ? "#fff" : "#fff", textDecoration: "none", border: `1px solid ${border}`, fontWeight: 700 }}>Go to Orders →</Link>
        <Link to="/#pricing" style={{ fontFamily: "DM Mono, monospace", fontSize: 12, padding: "9px 14px", borderRadius: 10, border: `1px solid ${border}`, background: cardBg, color: fg, textDecoration: "none" }}>View pricing</Link>
        <button onClick={fetchAll} style={{ fontFamily: "DM Mono, monospace", fontSize: 12, padding: "9px 14px", borderRadius: 10, border: `1px solid ${border}`, background: cardBg, color: muted, cursor: "pointer" }}>Refresh plans</button>
      </div>
